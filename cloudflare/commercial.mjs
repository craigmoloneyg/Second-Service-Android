...
export async function handleCommercial(request,env){const url=new URL(request.url);if(!url.pathname.startsWith('/api/billing/')&&!url.pathname.startsWith('/api/consultant/')&&!url.pathname.startsWith('/api/myob/')&&!url.pathname.startsWith('/api/documents/'))return null;if(!env.DB)return json({error:'Storage unavailable.'},503);await tables(env);try{
 ...
 if(url.pathname==='/api/myob/status'&&request.method==='GET'){const r=await env.DB.prepare('SELECT business_id,last_sync_at,snapshot,CASE WHEN cf_token IS NOT NULL AND length(cf_token)>0 THEN 1 ELSE 0 END AS company_login_saved FROM p2p_myob_connections WHERE account_id=?').bind(u.id).first();return json({connected:!!r,business_id:r?.business_id||null,last_sync_at:r?.last_sync_at||null,snapshot:r?.snapshot?JSON.parse(r.snapshot):null,company_login_saved:Boolean(r?.company_login_saved),configured:Boolean(env.MYOB_CLIENT_ID&&env.MYOB_CLIENT_SECRET)});}
