...
function build(){
 ...
 <div class="acc-note">First release: cash-basis GST working papers and recorded wage payments. Automatic PAYG is available for supported regular payments dated 1 July 2026–30 June 2027. Award interpretation, leave accrual, income tax returns, BAS/STP submission and payments are not enabled.</div>
 ...
 <p>Enter earnings, choose the employee’s declared tax treatment, then calculate PAYG and take-home pay. Check the applicable pay rates and super earnings base/rate. Record the payment only after paying it. Garnish does not send wages, super or STP reports.</p>
 ...
 <label>Tax calculation<select name="withholding_mode"><option value="automatic">Automatic PAYG · 2026–27</option><option value="manual">Enter verified withholding manually</option></select></label>
 <div id="acc-auto-tax"><div class="acc-grid">
 <label>Pay frequency<select name="pay_frequency" required><option value="">Choose…</option><option value="weekly">Weekly</option><option value="fortnightly">Fortnightly</option><option value="monthly">Monthly</option><option value="quarterly">Quarterly</option></select></label>
 <label>Employee tax treatment<select name="tax_scale" required><option value="">Choose from their declaration…</option><option value="2">Australian resident · tax-free threshold claimed</option><option value="1">Australian resident · no tax-free threshold</option><option value="3">Foreign resident · not a working holiday maker</option><option value="5">Threshold claimed · full Medicare exemption</option><option value="6">Threshold claimed · half Medicare exemption</option></select></label>
 <label>HELP / other study or training loan<select name="study_loan" required><option value="">Choose…</option><option value="false">No declared debt</option><option value="true">Declared debt</option></select></label>
 ${amount('Annual tax offset declared ($)','annual_tax_offset','value="0"')}
 </div><p>Uses ATO Schedules 1 and 8 effective 1 July 2026. Medicare is included where applicable. Enter offsets only from the employee’s withholding declaration. No TFN number is stored here.</p>
 <label class="acc-check"><input type="checkbox" name="declaration_confirmed" required>I checked the employee’s declaration: they provided a TFN and this is regular pay. No working-holiday visa, seniors/pensioner treatment, family Medicare adjustment, withholding variation, salary sacrifice, bonus/backpay, termination payment or daily/casual tax-table treatment applies.</label>
 <p>For any excluded treatment, select verified manual withholding. If there are 53 weekly or 27 fortnightly pays this year, discuss additional withholding with the employee and use manual mode if requested.</p></div>
 <div class="acc-grid">${amount('Manual PAYG including any study-loan withholding ($)','payg')}${amount('After-tax deductions ($)','deductions','value="0"')}${amount('Eligible super earnings base ($)','super_base')}${amount('Verified super rate (%)','super_rate','max="100"')}</div>
 ...
 const payInput=()=>({...fields($('acc-payroll-form')),lines:lines('acc-payroll-lines'),verified:$('acc-payroll-form').elements.verified.checked,study_loan:$('acc-payroll-form').elements.study_loan.value==='true'?true:$('acc-payroll-form').elements.study_loan.value==='false'?false:null,declaration_confirmed:$('acc-payroll-form').elements.declaration_confirmed.checked});
 const syncTaxMode=()=>{const f=$('acc-payroll-form');const auto=f.elements.withholding_mode.value==='automatic';$('acc-auto-tax').hidden=!auto;$('acc-auto-tax').querySelectorAll('input,select').forEach(el=>el.disabled=!auto);f.elements.payg.disabled=auto;f.elements.payg.required=!auto;f.elements.payg.parentElement.hidden=auto;};
 $('acc-payroll-form').addEventListener('change',()=>{syncTaxMode();$('acc-pay-result').textContent='';});
 $('acc-payroll-form').addEventListener('input',()=>{$('acc-pay-result').textContent='';});
 $('acc-payroll-form').addEventListener('reset',()=>setTimeout(syncTaxMode,0));syncTaxMode();
 ...
 $('acc-pay-preview').onclick=async()=>{try{const {preview:p}=await api('payroll-preview',payInput());$('acc-pay-result').innerHTML=`<div class="acc-grid">${metric('Gross',p.gross)}${metric('Total PAYG withheld',p.payg)}${p.withholding?metric('Included study-loan withholding',p.withholding.study_loan_withholding):''}${metric('Net payment',p.net)}${metric('Employer super',p.super)}${metric('Employer cost',p.gross+p.super)}</div>`;}catch(e){$('acc-pay-result').textContent='';message(e.message,true);}};
...
async function refreshReport(){
 ...
 <h3>Payroll review · not lodged</h3><p>Recorded payments and reversals for the dates above. Gross and PAYG match the W1/W2 working figures. Super shown is the amount recorded, not confirmation it has been paid. This is not an STP upload file.</p>
 ${r.payroll_review?`<div class="acc-grid">${metric('Gross wages',r.payroll_review.totals.gross)}${metric('PAYG including study loans',r.payroll_review.totals.payg)}${metric('After-tax deductions',r.payroll_review.totals.deductions)}${metric('Net wages',r.payroll_review.totals.net)}${metric('Recorded super',r.payroll_review.totals.super)}</div><p>${r.payroll_review.balance_difference===0?'Gross less PAYG and deductions equals net wages.':'Review required: wage totals do not balance.'}</p><div class="acc-actions"><button type="button" id="acc-payroll-export">Download payroll review CSV</button></div>`:''}
 <details><summary>What remains before ATO reporting?</summary><p>Garnish still needs developer registration, security assessment, ATO testing and production approval. STP also needs employer and employee identifiers, payroll classifications, year-to-date reporting and correction handling. These recorded-payment totals do not establish STP readiness.</p><p><a href="https://softwaredevelopers.ato.gov.au/OnlineservicesforDSPs" target="_blank" rel="noopener noreferrer">Open official ATO developer registration</a></p></details>
 ...
 if(r.payroll_review)$('acc-payroll-export').onclick=()=>{
 const p=r.payroll_review;
 const headers=['Record ID','Payment date','Employee / reference','Pay reference','Period start','Period end','Type','Reverses record','Reversed by record','Gross AUD','PAYG AUD (includes study loans)','Deductions AUD','Net AUD','Super AUD','Included study loan AUD (blank if unknown)','Calculation'];
 const rows=[['Garnish payroll working records — NOT LODGED — NOT AN STP UPLOAD'],['Business',book.settings?.business_name||''],['From',start],['To',end],['Coverage','Recorded payments and dated reversals only. Not a payslip or income statement. Check opening balances, employee identity, special payments and super payment status.'],headers,
 ...p.rows.map(x=>[x.id,x.paid_date,x.employee,x.reference,x.period_start,x.period_end,x.type,x.reversal_of,x.reversed_by,decimal(x.gross),decimal(x.payg),decimal(x.deductions),decimal(x.net),decimal(x.super),x.study_loan==null?'':decimal(x.study_loan),x.calculation]),
 ['TOTAL','','','','','','','','',decimal(p.totals.gross),decimal(p.totals.payg),decimal(p.totals.deductions),decimal(p.totals.net),decimal(p.totals.super),'','']];
 download('Garnish-payroll-review-'+start+'-'+end+'.csv',rows.map(row=>row.map(csv).join(',')).join('\r\n'),'text/csv');
 };
