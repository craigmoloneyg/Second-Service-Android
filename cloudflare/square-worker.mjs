...
async function issueSession(env,account,email){
  ...
  await env.DB.prepare("INSERT INTO garnish_sessions(token,account_id,expires_at) VALUES(?,?,?)").bind(token,account.id,expires).run();
...
async function directAuth(request,env,action){
  try{
    ...
    let account=await env.DB.prepare("SELECT * FROM garnish_accounts WHERE lower(email)=?").bind(email).first();
    ...
    if(action==='signup'){
      if(account){
        ...
        const s=await env.DB.prepare("SELECT COUNT(*) n FROM garnish_sessions WHERE account_id=? AND expires_at>?").bind(account.id,new Date().toISOString()).first();
        ...
        await env.DB.prepare("UPDATE garnish_accounts SET password_hash=?,password_salt=?,workspace_id=? WHERE id=?").bind(hash,salt,workspace,account.id).run();
      ...
      await env.DB.prepare("INSERT INTO garnish_accounts(id,email,password_hash,password_salt,workspace_id,created_at) VALUES(?,?,?,?,?,?)").bind(id,email,hash,salt,workspace,created).run();
...
async function directSession(request,env){
  try{
    ...
    let auth=null;
    if(token)auth=await env.DB.prepare("SELECT a.email,a.workspace_id FROM garnish_sessions s JOIN garnish_accounts a ON a.id=s.account_id WHERE s.token=? AND s.expires_at>?").bind(token,new Date().toISOString()).first();
